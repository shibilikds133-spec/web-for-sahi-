-- Fix get_public_leaderboard to only include results where public_visible IS TRUE
-- This ensures that points from results that are only "Admin Published" do not leak into the public point table.
...
BEGIN
  ...
  -- 4. Execute with corrected hierarchy-aware org filter and public_visible filter
  RETURN QUERY
