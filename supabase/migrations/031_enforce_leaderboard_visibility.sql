-- ============================================================
-- Migration 031: Enforce Leaderboard Visibility (v4)
-- Hierarchy-aware: scopes by festival ownership, not results.tenant_id
-- Sector festival → Unit results (child orgs)
-- Division festival → Sector results (child orgs)
-- District festival → Division results (child orgs)
-- ============================================================
...
BEGIN
  ...
  --    If owner org exists → include results from orgs that are DIRECT CHILDREN of it
  --    (Sector → shows Units; Division → shows Sectors; etc.)
  --    If no owner org found (e.g., super_admin) → include ALL results for this festival
  RETURN QUERY
  SELECT
